// All issuesemployee-name
let currentIssues = [
    {
        id: "qm3bd",
        description: "The data is invalid. [ERROR_INVALID_DATA (0xD)]",
        date: "13 September 2023,11:33:21 PM",
        priority: "high",
        employee: "Shanuga",
        status: "opened",
    },
...
form.addEventListener("submit", (e) => {
    ...
    if (
        ...
        if (fileInput.files.length > 0) {
            ...
            formData.append("fileURL", fileURL); // Append file URL to display in the column

            // Add other form data to the FormData object
            formData.append("description", description_input__value);
            formData.append("priority", priority_input.value);
            formData.append("assigned", assigned_input.value);

            // Additional properties
            formData.append("id", uniqueId());
            formData.append("date", currentTimeDate());
            formData.append("status", "opened");

            // Perform AJAX request or other processing with formData
            // ...

            // Clear the file input after submission
            fileInput.value = "";

            // Created temporary object using all values including fileURL
            let inputValue_temp = {
                id: uniqueId(),
                ...
                status: "opened",
                fileURL: fileURL, // Add fileURL to the object
            };

            // New issue row created
            issueRowCreate(inputValue_temp);

            // Newly created object pushed in the main array
            ...
            // If the search field is opened, after adding new data the search field will close
            search_field_close();
            ...
            // Cleared the form after a successful issue entry
            // form.reset();
        } else {
            // File not selected error
            document.querySelector(".fileUploadSection small.error").innerHTML =
                "Upload a file";

            // Add error sign
            error_addRemove([fileInput], "false");
        }
    } else {
...
function issueRowCreate(issue) {
    let rowItems = `
        ...
        <div class="issue_date styled-date">
        <p>${issue.date}</p>
    </div>
        ...
        <!-- New column for the issue image -->
        <div class="issue-image">
            <p>${issue.fileURL ? `<img src="${issue.fileURL}" alt="Uploaded File" style="max-width: 50%; height: auto;">` : ''}</p>
